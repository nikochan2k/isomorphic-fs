...
export const testAll = (fs: FileSystem) => {
  ...
  test("add empty file", async () => {
    ...
    let [stats, e] = await file.stat();
    if (stats) fail("Found file: " + file.path);
  ...
  test("read text file", async () => {
    ...
    const buffer = ret as ArrayBuffer;
  ...
  test("continuous read and write", async () => {
    ...
    const [rs] = await file.createReadStream();
    let [ret] = await rs.read(6);
    let buffer = ret as ArrayBuffer;
    let text = await c.toText(buffer);
    expect(text).toBe("大谷");

    await rs.seek(6, SeekOrigin.Begin);
    [ret] = await rs.read();
    buffer = ret as ArrayBuffer;
  ...
  test("mkdir test", async () => {
    ...
    const [stats, e] = await folder.stat();
    if (stats) fail("Found folder: " + folder.path);
    expect(e.name).toBe(NotFoundError.name);
    await folder.mkdir();
    await folder.stat();
  ...
  test("create file in dir", async () => {
    ...
    const [rs] = await file.createReadStream();
    let [ret] = await rs.read();
    const inBuf = ret as ArrayBuffer;
